...
/**
 * Summary component for Warp Bridge transfer_mint_authority instruction
 */
export const BridgeTransferMintAuthoritySummary: React.FC<InstructionSummaryProps> = ({
  ...
  const newAuthority = instruction.accounts?.[4]?.pubkey;
  ...
  return (
    <div className="space-y-3 text-sm">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            Transfer {tokenSymbol} mint authority (irreversible)
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-border/50 bg-muted/30 p-3">
        ...
        {newAuthority && <AddressWithButtons address={newAuthority} label="New Authority" />}
      </div>
    </div>
  );
};
