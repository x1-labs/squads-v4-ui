  ...
  Object.entries(envVars).forEach(([key, value]) => {
    if (value && typeof value === 'string' && value.length > 0) {
      ...
      envSquads.push({
        address: value,
        name: name,
        addedAt: 0, // Use 0 to indicate it's from env, not user-added
      });
    }
  });
...
export function mergeEnvSquadsWithSaved(savedSquads: SavedSquad[]): SavedSquad[] {
  ...
  // First add env squads (they take precedence)
  envSquads.forEach(squad => {
    squadMap.set(squad.address, squad);
  });
  
  // Then add saved squads that aren't already present
